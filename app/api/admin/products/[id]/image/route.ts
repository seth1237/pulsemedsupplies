...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  const relativePath = `/products/uploads/${id}.webp`
  const diskPath = path.join(process.cwd(), 'public', 'products', 'uploads', `${id}.webp`)
  await mkdir(path.dirname(diskPath), { recursive: true })
  await writeFile(diskPath, optimized)

  const product = await updateLocalProduct(id, { image: relativePath })
  if (!product) {
